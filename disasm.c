...
static char *opcodes = NULL;
static int opcode_len = 0;
...
//====================================================================================================
int FindCommand(char* p, char* modrm) {	//MOD-REG-R/M
...
//====================================================================================================
int ParsePrefixes(SCommand* cp, const uint8* mp, int count) {
	char buf = 0;
	int bytecounter = 0;
...
//====================================================================================================
int GetCommand(SCommand* cp, uint8 mp[], int com_len1) {
	UByte temp_byte;
	int bytecounter = 0, temp_int, ext_len = 0, counter = 0, com_len;
	uint8 * temp_char, * temp_char1, * temp_char2, * temp_char3;
	uint8 buf, buf2;
	int offset = 0;
	...
	while (counter < cp->par_count) {
		...
		case 'c'://cr0-7
			...
			break;
		case 'm'://memory: 0x67, 0x66, segment redefinition prefixes affects
		...
		case 'g'://JMP FAR, CALL FAR
			...
			cp->parameters[counter] -= 4;
			break;
		case 'x'://xlat: byte ptr ds[ebx/bx+al]
			ext_len = 0;
			cp->parameters[counter][0] = 'B';
			cp->parameters[counter][1] = 'Y';
			cp->parameters[counter][2] = 'T';
		...
		case 'q'://q:rep/repnz влияют на описание команды: modification
			cp->par_count--;
		...
		case 'k'://Call/jmp far intersegment — memory operand redefinition
			cp->par_count--;
...
void printCommand(SCommand *command){
...
int loadOpCodes(){
	const static char *opfilename = "res/opcodes.ttt";
	FILE* hFileOpcodes;
	if ((hFileOpcodes = fopen(opfilename, "rb")) == 0) {
...
int disasm(uint8 cmd[], int cmd_len){
	...
	if(RET_SUCCESS == GetCommand(&command, cmd, cmd_len)){
		printCommand(&command);
	}
	return  0;
}
